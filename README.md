...
Проект построен по принципам **Clean Architecture** (domain / application / api слои)
и паттернам DRY, KISS, SOLID.
...
```bash
make makemigrations name="описание"   # сгенерировать миграцию
make migrate                          # применить
make rollback                         # откатить на один шаг
```
...
```bash
# Логи
make logs            # все backend-сервисы
make logs-api        # только API (есть logs-bot / logs-scheduler / logs-logbot)

# Shell внутри контейнера
make shell-api       # (есть shell-bot / shell-logbot)

# Перезапуск с пересборкой
make restart-api     # restart-bot / restart-scheduler / restart-logbot / restart-front

# Остановка
make down            # backend + infra
```
...
cd frontend
bun install
bun run dev          # запуск Hono-сервера
bun run css          # пересборка Tailwind в watch-режиме
```
...
Настройки: `ruff.toml`, `pyproject.toml` (line-length 100, target py312, strict mypy).

---

## Лицензия

Личный проект. Все права принадлежат автору.
